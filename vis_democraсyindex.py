    ...
    def build_input(self, k, stimul, time):
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                if democracy[i] == democracy[j]:
                    continue
                matrix[i][j] = (democracy[i] - democracy[j]) ** 4
                #matrix[i][j] = democracy[i] * democracy[j] + human_index[i] * human_index[j]
                #matrix[i][j] += stimul
...
#         ax.add_feature(cartopy.feature.RIVERS)
    ...
    def print_coalitions(self):
        pass
        for i in range(len(self.coalition_to_countries)):
            c = self.coalition_to_countries[i]
            print(f"размер коалиции {i + 1}: {len(c)}")
            print(c)
...
df.build_input(k=6, stimul=0, time = 10)
df.format_output()
df.plot_output()
df.print_coalitions()