...
big_table.fillna(0) # replace nan with 0
...
with open("Data/Composite Index of National Capability.csv") as file:
    for line in file:
        ...
        natcap[iso] = float(value) + 1.1
        if natcap[iso] > maxnatcap:
            maxnatcap = natcap[iso]


dist = [[0.0] * n for i in range(n)]
height = big_table.shape[0]

median_dist = big_table['dist'].median()
...
for line in values:
    ...
    #d = math.log(median_dist + 1) / mx_dist
    #d = 0
    #if line[col_to_ind['dist']] + 1 > 0:
    d = math.log(line[col_to_ind['dist']] + 1) / mx_dist
    if (iso1, iso2) in smctry:
        smc = smctry[(iso1, iso2)]
    else:
        smc = 0
    nc1 = math.log(natcap[iso1]) / math.log(maxnatcap)
    nc2 = math.log(natcap[iso2]) / math.log(maxnatcap)
    
    # (1 - line[col_to_ind['comcol']])

    dist[ind1][ind2] = nc1 * nc2 * \
                            (line[col_to_ind['contig']] + \
                                line[col_to_ind['comlang_off']] + \
                                line[col_to_ind['comlang_ethno']] + \
                                    (1 - line[col_to_ind['comrelig']]) + d + smc)

    
    if math.isnan(dist[ind1][ind2]):
        dist[ind1][ind2] = (d + 1) * nc1 * nc2
    if math.isnan(dist[ind1][ind2]):
        dist[ind1][ind2] = 1
...
build_input(k=4, dist_matrix = dist)
